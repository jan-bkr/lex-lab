...
- **Öffentliche Supabase-Selects**: niemals `select('*')` auf `tools` im Browser verwenden — explizite Spaltenliste ohne `submitted_by` pflegen. DB-seitig wurde `submitted_by` via column-level REVOKE für `anon` gesperrt (nur noch INSERT/UPDATE/REFERENCES verbleiben, kein SELECT).
