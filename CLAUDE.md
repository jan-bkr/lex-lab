...
- [x] **RLS-Bypass-Fix: anon INSERT auf `tools` + anon SELECT auf `newsletter_subscribers`** (2026-04-15) — Zwei veraltete RLS-Policies wurden als Migrations dokumentiert und müssen im Supabase SQL Editor ausgeführt werden: (1) anon INSERT-Policy auf `tools` (`"Anyone can submit"`) entfernt — der Submit-Flow läuft vollständig serverseitig via `adminSupabase` mit eigenen Security-Checks; direkte anon-INSERTs umgehen Rate Limit, Validierung und Origin-Check. (2) Public SELECT-Policy auf `newsletter_subscribers` entfernt — alle Zugriffe (subscribe, unsubscribe, Admin-UI) nutzen `adminSupabase`; ein öffentliches SELECT legt die gesamte E-Mail-Liste für jeden Inhaber des öffentlichen anon-Keys offen (DSGVO-Verstoß). Migrations: `20260415000003` + `20260415000004`, SQL in `RUN_IN_DASHBOARD.sql` ergänzt. **Operativer Nachschritt: SQL im Supabase Dashboard ausführen.**
...
- Supabase-Migrationen `20260415000003` + `20260415000004` **müssen noch im Supabase SQL Editor ausgeführt werden**: DROP anon-INSERT-Policy auf `tools` + DROP anon-SELECT-Policy auf `newsletter_subscribers` — SQL in `RUN_IN_DASHBOARD.sql` (Abschnitte 3 + 4). Nach Ausführung: `SELECT policyname FROM pg_policies WHERE tablename IN ('tools','newsletter_subscribers');` zur Verifikation.
