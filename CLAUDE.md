...
- [x] **Workflow Premium UI-Paket** (2026-04-16) — Workflow-Karten vollständig klickbar (Link-Wrapper + group-Hover), WORKFLOW_TEASERS aufgewertet (raffinierteres "Bald verfügbar"-Badge, gedimmt, präzisere Typografie), `WorkflowStepsTeaser` auf Detailseite vollständig überarbeitet: progressiver Opacity-Fade über Schritte, Gradient-Overlay, strukturierter Header mit "In Vorbereitung"-Badge, Inline-Newsletter-CTA mit Arrow statt Text-Link, Excerpt-Kasten mit blauem Left-Border-Akzent
...
- [x] **Compliance, Inhaltswahrheit & Admin-Korrektheit** (2026-04-15) — Datenschutz-Seite ehrlich dokumentiert (Vercel Analytics, Upstash Redis, DSGVO Art. 6 I lit. f). Newsletter: Mail-first-Reihenfolge (DB-Write nur bei erfolgtem Mail-Versand). Mock-Fallbacks aus `ToolDetailClient.tsx` und `WorkflowDetailClient.tsx` entfernt. `generateMetadata` filtert jetzt nach `status=approved` (Tools) resp. `published=true` (Workflows). `requireAdminSession()` auf alle 6 Admin-Read-Actions ausgeweitet. `pricing`-Spalte end-to-end vereinheitlicht (Admin schrieb zuvor in falsche Spalte) — Hinweis: späterer Fix (2026-04-15) korrigierte den Spaltennamen erneut, da die DB-Spalte tatsächlich `pricing` heißt, nicht `pricing_type`. `revalidatePath` in `updateTool` nutzt jetzt `payload.slug` statt interner UUID.
...
- [x] **Admin-Allowlist, öffentliche Datenabfragen & Vote-Härtung** (2026-04-15) — `proxy.ts` und `admin/layout.tsx` prüfen jetzt `ADMIN_EMAIL` konsistent (wie `requireAdminSession()`), sodass kein gewöhnlicher Supabase-User Admin-Seiten sehen kann. `tools/page.tsx` und `ToolDetailClient.tsx`: `select('*')` durch explizite Spaltenlisten ersetzt — `submitted_by` wird nicht mehr an den Browser übertragen. `ToolDetailClient.tsx`: lädt jetzt nur das angefragte Tool per `.eq('slug', slug).maybeSingle()` statt alle approved Tools; ähnliche Tools werden separat mit minimalen Spalten (`id, name, slug, rechtsgebiet`) via `.overlaps()` geladen. `vote/route.ts`: prüft vor dem RPC-Aufruf ob das Tool `approved` ist — Votes auf nicht-öffentliche Tools werden mit 404 abgewiesen. Supabase-DB direkt gehärtet: column-level SELECT auf `tools.submitted_by` für `anon` revoked (REVOKE table-level + GRANT explizite Spaltenliste); anon INSERT-Policy `"Anyone can submit comment"` auf `tool_comments` gedroppt. `ADMIN_EMAIL` in Vercel Environment Variables gesetzt.
...
- [x] **Tool Finder Premium-Inszenierung + Workflows Teaser-Seite** (2026-04-16) — (1) `FinderPanel` als wiederverwendbare Shared-Komponente extrahiert (`src/components/FinderPanel.tsx`): weißes Panel mit `border-blue-100 shadow-sm`, "Empfohlen"-Badge (Sparkles-Icon + blauer Pill), Display-Headline, `compact`-Prop für Tools-Seite. (2) `/tools`: FinderPanel ersetzt die alte Link-Box; "Tool einreichen" von primärem Blau-Button auf Ghost-Button zurückgestuft — klare Hierarchie Finder > Submit. (3) Homepage: FinderPanel als eigene Section zwischen Hero und "Tools der Woche" — prime Platzierung ohne Hero-Überladung. (4) `/workflows` Leer-State: "Noch keine Workflows verfügbar"-Text ersetzt durch 3 hochwertige hardcodierte Teaser-Cards (M&A / Gesellschaftsrecht / Steuerrecht) mit "Erscheint in Kürze"-Badge, border-l-Farbakzent, Lock-Icon und Lesezeit — exakt dieselbe Qualität wie `/beitraege`. Newsletter-CTA am Seitenende. Wenn DB Workflows enthält, erscheinen diese wie gehabt über dem Leer-State.
